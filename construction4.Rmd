...
---
```{r}
list.fileDesc <- list.files("../resuvou4_ANGLE_B/")

file1 <- read.delim(paste("../resuvou4_ANGLE_B/", list.fileDesc[1], sep="/"),  header=TRUE, row.names = 1)
...
for(i in 1:length(list.fileDesc)){
  file1 <- read.delim(paste("../resuvou4_ANGLE_B/", list.fileDesc[i], sep="/"),  header=TRUE, row.names = 1)
...
write.table(resun, file="matrice_angB_1020Struct.csv", col.names = T, row.names=T, quote=FALSE,sep = ",")
toto<- read.csv("matrice_angB_1020Struct.csv")
resun<- read.csv("matrice_angB_1020Struct.csv")
```
...
```{r}
resun <- read.table("~/Bureau/projet_long/analyse2/matrice_dist_1020Struct.csv", comment.char="", sep=";")
```
...
resu2 <- resu[,-var.Supp]
dim(resu2)
write.table(resu2, "matrice_angleB_filter.csv")
resfffff <- read.table("~/Bureau/projet_long/analyse2/matrice_angleB_filter.csv", quote="\"", comment.char="")
```
...
plot(pca.res2, choix="var", label = "none")
```
...
```{r}
resA <- read.table("~/Bureau/projet_long/analyse2/matrice_angleA_filter.csv", quote="\"", comment.char="")
resB <- read.table("~/Bureau/projet_long/analyse2/matrice_angleB_filter.csv", quote="\"", comment.char="")
dim(resA)
dim(resB)
resA_B<- merge( resA, resB ,row.names = 1 )
dim(resA_B)
which(is.na(resA_B),arr.ind=TRUE)
```
...
```{r}
> write.table(resff, file="matrice_angA_B_1020Struct.csv", col.names = T, row.names=T, quote=FALSE,sep = ",")
> toto<- read.csv("matrice_angA_B_1020Struct.csv")
> View(toto)
```
...
```
```{r}
res <- read.table("~/Bureau/projet_long/analyse2/matrice_dist_angA_B_nonfilter_1020Struct.csv", quote="\"", comment.char="", sep =',')
...
```{r}
tmp <- which(cor_X_sansdiag>0.80, arr.ind=T)
couple.desc.cor <- paste(colnames(cor_X_sansdiag)[tmp[,1]], colnames(cor_X_sansdiag)[tmp[,2]], sep="-")
```

```{r}
couple.desc.cor
list0=list(couple.desc.cor[1])
```
